...
# 		return tf.keras.backend.in_train_phase(samples, discrete_logits)
...
class Sample_Concrete(Layer):
    ...
    def call(self, logits):
        ...
        d = tf.shape(logits_)[2]
        uniform = tf.random.uniform(shape=(batch_size, self.k, d),
                                    minval=np.finfo(tf.float32).tiny,
                                    maxval=1.0)
...
def L2X(datatype, train = True): 
	...
	filepath = os.path.join(current_dir, 'models', datatype, 'L2X.hdf5')
