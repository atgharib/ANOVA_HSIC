...
class HSICNetSparseMax(nn.Module):
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, sigma_init_X, sigma_init_Y):
        super(HSICNetSparseMax, self).__init__()
    ...
    def forward(self, x):
        ...
        self.importance_weights = self.sparsemax(self.fc3(x))
...
class HSICNetGumbelSoftmax(nn.Module):
    ...
    def forward(self, x):
        ...
        self.importance_weights = self.gumbel_softmax_sampling(logits, temperature=.1, num_samples=5)
...
class HSICNetGumbelSparsemax(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply Gumbel-Softmax sampling with 5 samples and temperature of 1
        self.importance_weights = self.gumbel_sparsemax_sampling(logits, temperature=10, num_samples=1)
    ...
    def gumbel_sparsemax_sampling(self, logits, temperature=0.1, num_samples=5):
        """
...
# Modified RBF Kernel for label y with learnable sigma
    ...
    # Training function
    def train_model(self, X, y, num_epochs=300, lr=0.001):
        ...
        for epoch in range(num_epochs):
            ...
            s, sigmas, sigma_y = self(X)  # importance weights from Gumbel-Softmax
            loss = -self.hsic_loss_adaptive(X, y, s, sigmas, sigma_y)  # Minimize negative HSIC
